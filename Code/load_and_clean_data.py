...
def load_image_data(image_features_dir = image_features_dir_default, standardize=0):
    train_features = np.asarray(pd.read_csv(image_features_dir + r'/train_features.csv').iloc[:, :])
    train_labels = np.asarray(pd.read_csv(image_features_dir + r'/train_labels.csv').iloc[:, 0])

    test_features = np.asarray(pd.read_csv(image_features_dir + r'/test_features.csv').iloc[:,:])
    test_labels = np.asarray(pd.read_csv(image_features_dir + r'/test_labels.csv').iloc[:, 0])
